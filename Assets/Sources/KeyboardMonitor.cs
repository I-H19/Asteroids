...
public class KeyboardMonitor : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (Input.GetKeyDown(KeyCode.Space)) ShootButtonDown?.Invoke();
        if (Input.GetKeyUp(KeyCode.Space)) ShootButtonUp?.Invoke();

        if (Input.GetKeyDown(KeyCode.LeftControl)) LaserButtonDown?.Invoke();
        if (Input.GetKeyUp(KeyCode.LeftControl)) LaserButtonUp?.Invoke();
    }
}
